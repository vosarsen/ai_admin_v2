...
-- Версия для текущей структуры БД (с company_id как text в dialog_contexts)
...
-- 10. DIALOG_CONTEXTS - Фильтрация по компании (TEXT версия)
-- Поскольку company_id имеет тип text, создаем соответствующий индекс
CREATE INDEX IF NOT EXISTS idx_dialog_contexts_company_text 
...
SELECT 
    'Индексы созданы!' as "Статус",
    COUNT(*) as "Всего индексов",
    pg_size_pretty(SUM(pg_relation_size(indexrelid))) as "Общий размер"
FROM pg_indexes 
JOIN pg_stat_user_indexes USING (schemaname, tablename, indexname)
WHERE schemaname = 'public' 
AND indexname LIKE 'idx_%';