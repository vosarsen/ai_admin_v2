...
**Обновлено:** 2025-11-19 v2 (updated pricing, removed tech talk)
...
// Стоимость AI
const aiCost = {
  pilotMonthly: 11990,          // Пилот (до 3 месяцев)
  pilot3Months: 35970,          // Пилот итого (3 × 11,990₽)
  regularMonthly: 19990,        // После пилота
  yearTotal: 215880             // Год: (11,990×3) + (19,990×9)
}
```
...
```javascript
const pilotROI = {
  cost: 35970,                    // Стоимость пилота (11,990₽ × 3)
...
pilotROI.realistic.roi()            // = 5.3x ROI
```
...
annualROI.conservative.roi()         // = 1.4x ROI (используем в оффере!)

annualROI.realistic.totalValue()     // = 759,000₽
annualROI.realistic.roi()            // = 3.5x ROI
```
...
```javascript
const breakeven = {
  pilotCost: 35970,              // Стоимость пилота (3 месяца)
...
KULTURA получил: 11 записей/месяц (2.2x больше порога)
```
...
function calculateSalonROI(salon) {
  ...
  const pilotCost = 11990 * 3      // = 35,970₽
  const regularCost = 19990 * 9    // = 179,910₽
  const yearCost = pilotCost + regularCost  // = 215,880₽
...
Это 2.2x ВЫШЕ порога окупаемости
...
ПОСЛЕ ПИЛОТА (если продолжите):
  ...
  → Продолжаете только если видите ROI

ЗА ГОД (если работаете 12 мес):
  35,970₽ (пилот) + 179,910₽ (9 мес) = 215,880₽
...
|-------------|-------------------------|--------------|-------------|--------|
| 1,000₽      | 12                      | 301,476₽     | 215,880₽    | 1.4x   |
| 1,500₽      | 8                       | 301,476₽     | 215,880₽    | 1.4x   |
| 2,000₽      | 6                       | 301,476₽     | 215,880₽    | 1.4x   |
| **2,500₽**  | **5**                   | **301,476₽** | **215,880₽**| **1.4x**|
| 3,000₽      | 4                       | 301,476₽     | 215,880₽    | 1.4x   |
| 4,000₽      | 3                       | 301,476₽     | 215,880₽    | 1.4x   |
| 5,000₽      | 2                       | 301,476₽     | 215,880₽    | 1.4x   |

**Паттерн:**
- ROI одинаковый (1.4x) - используем KULTURA proven value
...
- 5 записей/месяц для окупаемости ПИЛОТА
- KULTURA: 11 записей/месяц (2.2x margin)
- Легко достижимо для большинства салонов

### 3. ROI честный но положительный
- 1.4x ROI за год (не 8x, не 20x)
- 2.1x ROI уже в первый месяц
- Pricing transparency (пилот + regular)
...
**Для создания оффера:**
1. ✅ Use conservative numbers (25,123₽/мес, 1.4x ROI)
2. ✅ Show breakeven (5 записей/мес)
3. ✅ Clear pricing (pilot → regular)
...
**Validation:** Conservative, KULTURA proof, honest ROI 1.4x
**Pricing:** Pilot 11,990₽/мес (3 мес), Regular 19,990₽/мес
