...
1. БЕЗ ПРИВЕТСТВИЯ если это продолжение диалога (${isConversationContinuation ? 'ДА, это продолжение' : 'нет, новый диалог'})
2. БЕЗ технических деталей (не упоминай команды, API, системы)
3. БЕЗ избыточной информации (только то, что спросил клиент)
4. ИСПОЛЬЗУЙ имя клиента, если оно известно
5. БУДЬ кратким и по делу
6. НЕ используй форматирование WhatsApp (без *, _, ~)
...
ВАЖНО:
- Адаптируй ответ под результаты команд
