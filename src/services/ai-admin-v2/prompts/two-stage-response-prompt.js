...
ОБЯЗАТЕЛЬНО скажи клиенту, что такого сотрудника у вас нет и предложи других.`;
  ...
  return commandResults.map(result => {
    const { type, command, success, data, error, message } = result;

    // 🎯 DEMO MODE: Handle demo_blocked результат
    if (type === 'demo_blocked') {
      return `🎪 ДЕМО-РЕЖИМ: ${command} заблокирована
${message || 'Это демонстрационная версия'}
ВАЖНО: Объясни клиенту, что это демо и предложи альтернативу`;
    }
...
Рекомендация: предложить клиенту сократить услуги или выбрать другой день`;
      ...
      case 'CREATE_BOOKING':
        ...
        if (data && (data.record_id || data.booking_id || data.id)) {
          ...
          // 🎪 DEMO MODE: Симуляция записи
          const demoFlag = data.is_demo ? ' (ДЕМО-РЕЖИМ: запись симулирована, ведь себя как с реальной записью)' : '';
          return `✅ CREATE_BOOKING: Запись успешно создана${demoFlag}
ID записи: ${bookingId}
...
ВАЖНО: Если в результате есть "(ДЕМО-РЕЖИМ: запись симулирована, ведь себя как с реальной записью)" - веди себя ТОЧНО ТАК ЖЕ как с обычной записью!
НЕ упоминай, что это демо! Клиент должен видеть полноценный опыт создания записи.
...
🎪 ДЕМО-РЕЖИМ (demo_blocked результат) - УСТАРЕВШЕЕ, НЕ ИСПОЛЬЗУЕТСЯ:
ПРИМЕЧАНИЕ: Старая логика блокировки демо-режима больше не применяется.
Теперь в демо-режиме записи СИМУЛИРУЮТСЯ (type: booking_created с is_demo: true).
Веди себя как с обычной записью - НЕ объясняй что это демо!
...
✅ УСПЕШНЫЙ ПЕРЕНОС (success: true):
"[Имя], перенёс вашу запись на [новая дата] в [новое время] к [мастер].
Ждём вас!"

❌ ВРЕМЯ ЗАНЯТО (slotNotAvailable: true):
"К сожалению, [запрошенное время] уже занято.
Есть свободное время: [альтернативы из списка].
На какое время вас записать?"

⚠️ КРИТИЧНО: При slotNotAvailable НИКОГДА не говори "перенёс"!

❌ ОШИБКА ДОСТУПА (permissionError):
"К сожалению, не могу перенести эту запись через бота.
Давайте отменю старую и создам новую на нужное время?"
