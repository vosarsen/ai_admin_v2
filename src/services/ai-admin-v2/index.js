...
class AIAdminV2 {
  ...
  /**
   * Создание mock контекста для демо-режима
  ...
  createDemoContext(demoData, phone) {
    return {
      ...
      services: demoData.services.map(s => ({
        id: s.id,
        title: s.title,
        price_min: s.price,
        price_max: s.price,
        category: { title: 'Услуги' }
      })),
      staff: demoData.staff.map(s => ({
        id: s.id,
        name: s.name,
        specialization: s.name.includes('Мастер') ? 'Стилист' : 'Мастер'
      })),
      client: null, // Новый клиент в демо
  ...
  buildSmartPrompt(message, context, phone, options = {}) {
    ...
    // DEMO MODE: Добавляем специальную инструкцию для демо-режима
    if (context.isDemo) {
      basePrompt += `\n\n⚠️ ВАЖНО - ДЕМО РЕЖИМ:
Это демонстрационный чат на сайте. Особые правила:

1. НЕ создавай реальные записи (команды CREATE_BOOKING игнорируй)
2. Когда клиент хочет записаться - объясни услуги и скажи: "📌 Это демо-версия. Для реальной записи свяжитесь с нами через форму на сайте"
3. Можешь показывать цены, рассказывать о мастерах, объяснять как работает бот
4. Используй данные из context.services и context.staff для ответов
5. Будь дружелюбным и показывай возможности AI-администратора
6. Не упоминай что ты в "демо-режиме" - веди себя естественно, просто не создавай записи

Цель демо: показать как ты общаешься и помогаешь клиентам, чтобы они захотели подключить бота.`;
    }
