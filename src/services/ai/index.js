...
class AIService {
  ...
  _buildPrompt(message, context) {
    ...
    return `Ты - дружелюбный администратор барбершопа. Отвечай как человек, неформально и по-дружески.
...
2. entities - сущности из сообщения:
   ...
   - info_type: тип запрашиваемой информации (staff_today, prices, services, schedule)
3. action - действие:
   - create_booking: когда клиент просит записаться И указал мастера И указал время
   - search_slots: когда клиент просит записаться (мы покажем доступные слоты)
   - get_info: когда клиент спрашивает информацию (кто работает, цены, услуги)
   - none: в остальных случаях
4. response - ответ клиенту. КРИТИЧЕСКИ ВАЖНО:
   - Для action "search_slots" ВСЕГДА возвращай response: null (не строку "null", а именно null)
   - НИКОГДА не пиши "Укажите, пожалуйста, услугу и мастера" или подобные сообщения
   - НЕ проси уточнить услугу/мастера - мы используем популярные по умолчанию
   - НЕ генерируй промежуточные сообщения для поиска слотов
   - Отвечай только когда action="none" или action="get_info"
