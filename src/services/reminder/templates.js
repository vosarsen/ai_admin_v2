...
const dayBeforeTemplates = [
  ...
  "Здравствуйте, {name}! {staff} готов к {service} завтра в {time} 💼",
...
const twoHoursTemplates = [
  ...
  "Приветствую Вас, {name}! Ждём вас сегодня в {time} на {service}. {staff} подготовил всё необходимое и ждёт Вас .",
  ...
  "Здравствуйте, {name}! Через 2 часа ({time}) ожидаем Вас на {service}. Вас встретит: {staff}.",
...
function formatServicesInCase(services, caseType) {
  ...
  for (const { service, count } of serviceCounts.values()) {
    ...
    if (count > 1) {
      ...
      if (service.declensions && service.declensions.plural && service.declensions.plural[caseType]) {
        ...
        if (caseType === 'accusative' || caseType === 'prepositional_na') {
          pluralForm = serviceName
            ...
            .replace(/([а-яё]+)ий\s+([а-яё]+)$/iu, '$1ие $2ы')       // мужской массаж -> мужские массажи
            .replace(/([а-яё]+)ую\s+([а-яё]+)ию$/iu, '$1ые $2ии')    // базовую процедуру -> базовые процедуры
            .replace(/([а-яё]+)ое\s+([а-яё]+)ие$/iu, '$1ые $2ия')    // базовое покрытие -> базовые покрытия
            // Существительное без прилагательного
            .replace(/([а-яё]+)ние$/iu, '$1ния')                      // моделирование -> моделирования, окрашивание -> окрашивания
        ...
        // Именительный падеж (что?)
          ...
          pluralForm = serviceName
            // Прилагательное + существительное
            .replace(/([а-яё]+)ая\s+([а-яё]+)а$/iu, '$1ые $2и')      // мужская стрижка -> мужские стрижки
            .replace(/([а-яё]+)ий\s+([а-яё]+)$/iu, '$1ие $2и')       // мужской массаж -> мужские массажи
            .replace(/([а-яё]+)ое\s+([а-яё]+)е$/iu, '$1ые $2я')      // базовое покрытие -> базовые покрытия
            // Существительное без прилагательного - используем РОДИТЕЛЬНЫЙ ПАДЕЖ после числительных 2-4
            // Это правило русского языка: "2 стола" (род. ед.), а не "2 столы" (им. мн.)
        ...
        // Дательный падеж (чему?)
          ...
          pluralForm = serviceName
            // Прилагательное + существительное
            .replace(/([а-яё]+)ой\s+([а-яё]+)е$/iu, '$1ым $2ам')     // мужской стрижке -> мужским стрижкам
            .replace(/([а-яё]+)ому\s+([а-яё]+)у$/iu, '$1ым $2ам')    // мужскому массажу -> мужским массажам
            // Существительное без прилагательного
            .replace(/([а-яё]+)нию$/iu, '$1ниям')                     // моделированию -> моделированиям
            .replace(/([а-яё]+)юру$/iu, '$1юрам')                     // маникюру -> маникюрам
            .replace(/([а-яё]+)ажу$/iu, '$1ажам');                    // массажу -> массажам
        }
      ...
      // После числительных 2, 3, 4 используется родительный падеж единственного числа
      // После 5+ используется родительный падеж множественного числа
      // Но у нас уже есть форма для нужного падежа (prepositional_na для "на что?")
      // Поэтому просто используем pluralForm как есть
