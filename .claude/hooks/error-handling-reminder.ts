...
function getFileCategory(filePath: string): 'backend' | 'database' | 'other' {
    // Backend detection (AI Admin v2 structure)
    if (filePath.includes('/src/services/') ||
        filePath.includes('/src/api/') ||
        filePath.includes('/src/queue/') ||
        filePath.includes('/src/workers/') ||
        filePath.includes('/src/integrations/') ||
        filePath.includes('/src/middlewares/') ||
        filePath.includes('/src/sync/') ||
        filePath.includes('/src/monitoring/')) return 'backend';

    // Database detection
    if (filePath.includes('/src/database/') ||
        filePath.includes('/prisma/') ||
        filePath.includes('/migrations/') ||
        filePath.includes('schema.prisma')) return 'database';
