...
### 🔴 CRITICAL - Fix Database Saving Error (July 30, 2025)
- [ ] **Исправить ошибку "Failed to save booking to database"**
  - [ ] Проверить структуру таблицы bookings в Supabase
  - [ ] Выяснить какие поля обязательные
  - [ ] Исправить сохранение после создания записи в YClients
  - [ ] Убедиться что booking_id сохраняется корректно

### 🔴 URGENT - Test Critical Fixes from First Real Client (July 30, 2025)
- [ ] **Протестировать сохранение контекста в Redis**
  - [ ] Создать диалог с паузой 7+ минут
  - [ ] Убедиться что AI помнит контекст разговора
  - [ ] Проверить что сохраняется lastService, lastStaff, clientName
  - [ ] Проверить TTL контекста (30 дней)
