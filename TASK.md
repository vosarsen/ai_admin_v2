...
### 🔴 URGENT - Test Critical Fixes from First Real Client (July 30, 2025)
- [x] **Протестировать сохранение контекста в Redis** ✅
  - [x] Создать диалог с паузой 7+ минут ✅
  - [x] Убедиться что AI помнит контекст разговора ✅
  - [x] Проверить что сохраняется lastService, lastStaff, clientName ✅
  - [x] Проверить TTL контекста (12 часов для full_context) ✅
- [ ] **Протестировать исправление извлечения имени**
  - [ ] Отправить "Стрижка" как отдельное сообщение
  - [ ] Убедиться что это НЕ сохраняется как имя
  - [ ] Проверить что реальные имена извлекаются корректно
  - [ ] Протестировать с именами из черного списка (услуги, дни)
- [ ] **Протестировать напоминания для близких записей**
  - [ ] Создать запись менее чем за 4 часа
  - [ ] Проверить что напоминания НЕ создаются
  - [ ] Создать запись за 5+ часов
  - [ ] Проверить что напоминания создаются корректно
...
### 🔴 High Priority - Testing Required (August 2, 2025)
- [ ] **Протестировать последние исправления AI**
  - [ ] Проверить что приветствие всегда появляется в первом сообщении
  - [ ] Убедиться что нет дублирования информации о мастерах
  - [ ] Проверить что фразы "Проверяю..." не появляются в ответах
  - [ ] Протестировать предложение альтернатив при занятых слотах
